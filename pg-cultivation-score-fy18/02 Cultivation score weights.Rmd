...
pool %>% filter(!is.na(CULTIVATION_SCORE) & CAMPAIGN_NEWGIFT_CMIT_CREDIT > 0) %>%
  ...
  labs(title = bquote('Cultivation score versus' ~ log[10] ~ 'campaign giving')
    , x = 'PG cultivation score', y = 'Campaign giving credit', color = 'MG prioritization model')
```
...
pool %>% filter(!is.na(CULTIVATION_SCORE)) %>%
  ...
  scale_x_continuous(breaks = 0:18, minor_breaks = NULL) +
  labs(title = bquote('Cultivation score versus' ~ log[10] ~ 'largest gift')
       , x = 'PG cultivation score', y = 'Largest gift or pledge payment', color = 'MG prioritization model')
```
...
That's a marginal increase in outsample MSE (`r calc_outsample_mse(clmaps, xval_inds, 'CAMPAIGN_NEWGIFT_CMIT_CREDIT') %>% mean() %>% round(4)` to `r calc_outsample_mse(clmaps2, xval_inds, 'CAMPAIGN_NEWGIFT_CMIT_CREDIT') %>% mean() %>% round(4)`, difference of `r scales::percent({calc_outsample_mse(clmaps2, xval_inds, 'CAMPAIGN_NEWGIFT_CMIT_CREDIT') %>% mean()}/{calc_outsample_mse(clmaps, xval_inds, 'CAMPAIGN_NEWGIFT_CMIT_CREDIT') %>% mean()} - 1)`) with many fewer predictors.
...
Rebuild all models using the entire dataset. Full results are in the [appendix](#appendix-final-models).
...
```
  ...
  * Unsurprisingly, all intercepts are positive. The campaign all predictors model is closest to 0, and the all predictor models have lower intercepts across the board.
  * Intrestingly, alumni status is negative for all but the largest gift regressed on PG checklist model (which was barely significant).
...
```
  ...
  * When predicting campaign giving, giving recency (gave in last 3) is more useful than previous MG behavior! Note that loyalty is also a much better predictor than the MG PR model.
  * Visits are also very impactful on average.
  * Most of the coefficients shared between models point in the same direction, with the notable exceptions of age and MG model tiers.
...
```
  ...
  * In all four models, age has only a small impact after controlling for the other predictors.
  * The nonlinear splines fits suggest that peak giving occurs in the late 40s or early 50s, and the age 60+ indicator does not provide information beyond that included in other predictors.
...
```
  ...
  * The PG score models use a binary indicator requiring at least one gift in the last three years, and at least 10 years of giving total, while the all predictors models separate these two components and treat them numerically.
  * Controlling for the effects of the other predictors, recency of giving has a large impact on overall campaign giving in the all predictors model.
  * Looking at years of giving, interestingly both lines cross 0 before 10, suggesting a smaller cutoff (7 or 8 years?) could be used in the PG checklist.
  * When modeling expected campaign giving it's certainly better to treat these variables separately. Largest transaction appears to have a less clear relationship (as seen in the above plots).
...
$$ E(\text{giving amount} ~ | ~ \text{donor status} = 1) $$
  ...
  * Consider a time series approach, where future giving over some reasonable cultivation period (2 years?) is estimated conditional on previous data.
...
## Final models {#appendix-final-models}
