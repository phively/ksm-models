...
calc_partial_resids <- function(model, inds, xname, df) {
  ...
  data.frame(
    ...
    , y.hat = update(model, formula = . ~ ns(parse(text = scoped_xname) %>% eval(), df = scoped_spline_df)) %>% fitted()
  ) %>% mutate(
    # Compute partial residuals
    y.partial.resid = {model %>% residuals(type = 'partial')}[, 
      paste0('ns(', parse(text = xname), ', df = ', df, ')')]
    # Regression on partial residuals
    , y.hat.partial = lm(y.partial.resid ~ ns(x, df)) %>% fitted()
  ) %>% return()
}
```
...
Effect of age per model

Effect of recent giving per model
