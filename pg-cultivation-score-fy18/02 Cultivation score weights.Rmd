...
# Compute MSE
clms_errs <- data.frame(
  insample = clms %>%
    lapply(function(x) calc_mse(y = model.frame(x)[, 1], yhat = predict(x))) %>%
    unlist()
  , outsample = clms_preds %>% lapply(function(x) calc_mse(y = x$truth, yhat = x$preds)) %>% unlist()
) %>% gather('type', 'MSE', 1:2)
...
clms_preds %>% lapply(function(x) calc_mse(y = x$truth, yhat = x$preds)) %>% unlist() %>% data.frame(MSE = .) %>%
  ggplot(aes(x = MSE)) +
  geom_density() +
  geom_text(y = seq(.1, 1, length.out = k - 1), label = 1:(k - 1), color = 'blue') +
  geom_vline(aes(xintercept = mean(MSE)), color = 'blue', linetype = 'dashed', alpha = .5) +
  geom_rug(color = 'blue') +
  labs(
    title = bquote('MSE across samples, mean =' ~
        .(clms_preds %>% lapply(function(x)
          calc_mse(y = x$truth, yhat = x$preds)) %>% unlist() %>% mean() %>% round(2)
        )
      )
  )
```
...
```{r}
# In-sample MSEs
clms_errs %>% filter(type == 'insample') %>%
  ggplot(aes(x = MSE)) +
  geom_density() +
  geom_text(y = seq(5, 75, length.out = k - 1), label = 1:(k - 1), color = 'blue') +
  geom_vline(aes(xintercept = mean(MSE)), color = 'blue', linetype = 'dashed', alpha = .5) +
  geom_rug(color = 'blue') +
  labs(
    title = bquote('MSE across samples, mean =' ~
        .(clms_errs %>% filter(type == 'insample') %>% select(MSE) %>% unlist() %>% mean() %>% round(2)
        )
      )
  )
```
```{r}
# Out-of-sample MSEs
clms_errs %>% filter(type == 'outsample') %>%
  ggplot(aes(x = MSE)) +
  geom_density() +
  geom_text(y = seq(.1, 1.5, length.out = k - 1), label = 1:(k - 1), color = 'blue') +
  geom_vline(aes(xintercept = mean(MSE)), color = 'blue', linetype = 'dashed', alpha = .5) +
  geom_rug(color = 'blue') +
  labs(
    title = bquote('MSE across samples, mean =' ~
        .(clms_errs %>% filter(type == 'outsample') %>% select(MSE) %>% unlist() %>% mean() %>% round(2)
        )
      )
  )
```

In-sample performance is extremely optimistic.

```{r}
# Merge the list of predicted results into one large data frame
clms_insample <- foreach(i = 1:length(clms), .combine = 'rbind') %do% {
  data.frame(
    model = i
    , preds = clms[[i]] %>% predict()
    , truth = model.frame(clms[[i]])[, 1]
  )
}
clms_outsample <- foreach(i = 1:length(clms), .combine = 'rbind') %do% {
  clms_preds[[i]]
}
```
```{r}
# Plot residuals vs fitted for in-sample data
clms_insample %>% mutate(residuals = truth - preds) %>%
...
clms_outsample %>% mutate(residuals = truth - preds) %>%
  ggplot(aes(x = preds, y = residuals, color = factor(model))) +
  geom_point(alpha = .1) +
  geom_smooth(se = FALSE) +
...
clms_outsample %>% mutate(st.resid = (truth - preds)/sd(truth - preds)) %>%
  ggplot(aes(sample = st.resid, color = factor(model))) +
  geom_qq(alpha = .05) +
  geom_qq_line() +
...
The quantile-quantile plot further illustrates the issue. The drift above the reference line to the left and below it to the right suggest less density than expected in the tails, which follows given that the range is bound -- it's not possible to give less than a nondonor or (for practical purposes) more than a 9-figure donor.

The story is completely different when looking only at those who actually gave:

```{r}
# In-sample Q-Q plot excluding nondonors
clms_insample %>% mutate(st.resid = (truth - preds)/sd(truth - preds)) %>% filter(truth > 0) %>%
  ggplot(aes(sample = st.resid, color = factor(model))) +
  geom_qq(alpha = .05) +
  geom_qq_line() +
  labs(title = 'In-sample Q-Q plot, donors only'
       , color = 'cross-validation sample')
```

```{r}
# Out-of-sample Q-Q plot excluding nondonors
clms_outsample %>% mutate(st.resid = (truth - preds)/sd(truth - preds)) %>% filter(truth > 0) %>%
  ggplot(aes(sample = st.resid, color = factor(model))) +
  geom_qq(alpha = .05) +
  geom_qq_line() +
  labs(title = 'Out-of-sample Q-Q plot, donors only'
       , color = 'cross-validation sample')
```

This is quite a bit nicer, suggesting only slight skewness (in different directions for in-sample versus out-of-sample, interestingly).
