...
# Plot R-squared
plot_r2 <- function(model_list) {
  model_list %>%
  lapply(., function(x) summary(x)$r.squared) %>% unlist() %>% data.frame(r.squared = .) %>%
...
plot_mses <- function(model_list, xval, truth) {
  mses <- data.frame(
    insample = model_list %>%
      ...
      unlist()
    , outsample = calc_preds(model_list, xval, truth) %>% lapply(function(x) calc_mse(y = x$truth, yhat = x$preds)) %>% unlist()
  ) %>% gather('type', 'MSE', 1:2)
  mses %>%
...
plot_resids <- function(model_list, xval, yname) {
  resids <- calc_resids(model_list, xval, yname)
  # Plot residuals vs fitted for in-sample data
  insample <- resids$insample %>% mutate(residuals = truth - preds) %>%
...
The full (and hard to read) results for each model are in the [appendix](#campaign-cultivation-results).
...
## All predictors {#campaign-all-predictors}
...
Full results are in the [appendix](#campaign-cultivation-all-results-1).
...
## Campaign cultivation results {#campaign-cultivation-results}
...
## Campaign cultivation all results 1 {#campaign-cultivation-all-results-1}

[Back](#campaign-all-predictors)
