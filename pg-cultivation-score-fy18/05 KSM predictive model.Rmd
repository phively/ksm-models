...
```{r}
lm_full <- lm(rv.amt ~ ., data = traindat)
 summary(lm_full)
```
...
Drop:
  ...
  * Yearly cash
  * NU committees
  * CRU giving segment variables
  * DAF gifts
  * Gave 10K flag
  * Bequests flag
  * Loyal cash percent
  * KLC count
  * Yearly gift club counts
  * Cash total
  * NGC total
  * First year giving
  * First year giving amounts
  * Max cash month
  * Spouse alumni flag
  * HH region
  * HH record is NA
  * Events attended KSM, NU
  * Prospect flags
  * Months assigned, present/past

```{r}
lm_trimmed <- lm_full %>% update(
  formula = rv.amt ~
    COMMITTEE_KSM_LDR +
...
```{r}
lm_mses <- data.frame(statistic = c('adj.r.sq', 'mse.in', 'mse.out'))
lm_mses <- cbind(
  lm_mses
  , lm_full = c(
    summary(lm_full)$adj.r.sq
    , calc_mse(y = traindat$rv.amt, yhat = lm_full$fitted.values)
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_full, newdata = holdoutdat))
  )
  , lm_trimmed = c(
    summary(lm_trimmed)$adj.r.sq
    , calc_mse(y = traindat$rv.amt, yhat = lm_trimmed$fitted.values)
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_trimmed, newdata = holdoutdat))
  )
)
```
```{r}
...
Trimmed splines model, arbitrarily choosing `r dfs %>% I()` degrees of freedom.
...
lm_splines <- lm(
  rv.amt ~
    ...
    GIFT_CLUB_NU_LDR_YRS +
    ns(GIFTS_ALLOCS_SUPPORTED, df = dfs) +
    ns(GIFTS_OUTRIGHTS_PAYMENTS, df = dfs) +
    ns(GIVING_CRU_TOTAL, df = dfs) +
    ns(GIVING_MAX_CASH_YR, df = dfs) +
    ns(GIVING_MAX_PLEDGE_AMT, df = dfs) +
    ns(GIVING_MAX_PLEDGE_FY, df = dfs) +
    HOUSEHOLD_CONTINENT +
    KSM_GOS_FLAG +
    LOYAL_5_PCT_ANY +
    ns(NGC_PFY1, df = dfs) +
    ...
    ns(UOR_LOWER_BOUND, df = dfs) +
    UPGRADE3_CASH +
    ns(VELOCITY3_LIN_NGC, df = dfs) +
    ns(VISITS_5FY, df = dfs)
  , data = traindat
)
summary(lm_splines)
```
...
lm_mses <- cbind(
  lm_mses
  , lm_splines = c(
    summary(lm_splines)$adj.r.sq
    , calc_mse(y = traindat$rv.amt, yhat = lm_splines$fitted.values)
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_splines, newdata = holdoutdat))
  )
)
```
```{r}
...
Splines greatly improve the fit, though the degrees of freedom clearly need to be tweaked.

### Ridge regression

First consider a ridge regression with all variables, including splines on the numeric ones.
...
lm_mses <- cbind(
  ...
  , lm_ridge = c(
    NA
    , calc_mse(y = traindat$rv.amt, yhat = predict(lm_ridge, s = 'lambda.1se', newdata = traindat))
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_ridge, s = 'lambda.1se', newdata = holdoutdat))
  )
)
```
```{r}
...
### Trimmed ridge regression

Next, consider a ridge regression starting with just the trimmed set of variables.

```{r, cache = TRUE}
# alpha = 0 is the ridge regression penalty
lm_ridge_trimmed <- cv.glmnet(
  rv.amt ~
    COMMITTEE_KSM_LDR +
    ns(CRU_PFY1, df = 4) +
    ns(CRU_PFY2, df = 4) +
    ...
    ns(CRU_PFY4, df = 4) +
    ns(CRU_PFY5, df = 4) +
    ns(EVALUATION_LOWER_BOUND, df = 4) +
    GIFT_CLUB_NU_LDR_YRS +
    ns(GIFTS_ALLOCS_SUPPORTED, df = 4) +
    ns(GIFTS_OUTRIGHTS_PAYMENTS, df = 4) +
    ns(GIVING_CRU_TOTAL, df = 4) +
    ns(GIVING_MAX_CASH_YR, df = 4) +
    ns(GIVING_MAX_PLEDGE_AMT, df = 4) +
    ns(GIVING_MAX_PLEDGE_FY, df = 4) +
    HOUSEHOLD_CONTINENT +
    KSM_GOS_FLAG +
    LOYAL_5_PCT_ANY +
    ns(NGC_PFY1, df = 4) +
    ...
    PROGRAM_GROUP +
    ns(RECORD_YR, df = 4) +
    ns(UOR_LOWER_BOUND, df = 4) +
    UPGRADE3_CASH +
    ns(VELOCITY3_LIN_NGC, df = 4) +
    ns(VISITS_5FY, df = 4)
  , data = traindat
  , alpha = 0
)
...
lm_mses <- cbind(
  ...
  , lm_ridge_trimmed = c(
    NA
    , calc_mse(y = traindat$rv.amt, yhat = predict(lm_ridge_trimmed, s = 'lambda.1se', newdata = traindat))
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_ridge_trimmed, s = 'lambda.1se', newdata = holdoutdat))
  )
)
```
```{r}
...
Taking variables out of the ridge regression model hurts performance.

### Ridge regression with splines

Consider a full ridge regression model with splines on the numeric variables.

```{r}
lm_ridge_spline <- cv.glmnet(
  rv.amt ~
    HOUSEHOLD_RECORD +
    PROGRAM_GROUP +
    PREF_ADDR_TYPE_CODE +
    HOUSEHOLD_CONTINENT +
    ns(GIVING_FIRST_YEAR, df = 4) +
    ns(GIVING_FIRST_YEAR_CASH_AMT, df = 4) +
    ns(GIVING_FIRST_YEAR_PLEDGE_AMT, df = 4) +
    ns(GIVING_MAX_CASH_AMT, df = 4) +
    ns(GIVING_MAX_PLEDGE_FY, df = 4) +
    ns(GIVING_MAX_PLEDGE_AMT, df = 4) +
    ns(GIVING_CASH_TOTAL, df = 4) +
    ns(GIVING_NGC_TOTAL, df = 4) +
    ns(GIVING_CRU_TOTAL, df = 4) +
    ns(GIFTS_ALLOCS_SUPPORTED, df = 4) +
    ns(GIFTS_FYS_SUPPORTED, df = 4) +
    ns(GIFTS_OUTRIGHTS_PAYMENTS, df = 4) +
    ns(DAF_GIFTS_AMT, df = 4) +
    ns(CASH_PFY1, df = 4) +
    ns(CASH_PFY2, df = 4) +
    ns(CASH_PFY3, df = 4) +
    ns(CASH_PFY4, df = 4) +
    ...
    ns(CRU_PFY5, df = 4) +
    CRU_GIVING_SEGMENT +
    ns(GIFT_CLUB_KLC_YRS, df = 4) +
    GIFT_CLUB_BEQUEST_YRS +
    ns(GIFT_CLUB_LOYAL_YRS, df = 4) +
    GIFT_CLUB_NU_LDR_YRS +
    ns(GIFT_CLUBS_CFY, df = 4) +
    ns(GIFT_CLUBS_PFY1, df = 4) +
    ns(GIFT_CLUBS_PFY2, df = 4) +
    ns(GIFT_CLUBS_PFY3, df = 4) +
    ns(GIFT_CLUBS_PFY4, df = 4) +
    ns(EVALUATION_LOWER_BOUND, df = 4) +
    ns(UOR_LOWER_BOUND, df = 4) +
    KSM_GOS_FLAG +
    ns(MONTHS_ASSIGNED, df = 4) +
    ns(PAST_GO_MONTHS_ASSIGNED, df = 4) +
    ns(PAST_GO_MONTHS_SINCE_ASSIGNED, df = 4) +
    ns(COMMITTEE_NU_DISTINCT, df = 4) +
    ns(COMMITTEE_NU_YEARS, df = 4) +
    ns(COMMITTEE_KSM_LDR, df = 4) +
    ns(EVENTS_ATTENDED, df = 4) +
    ns(EVENTS_YRS, df = 4) +
    ns(EVENTS_PREV_3_FY, df = 4) +
    ns(KSM_EVENTS_ATTENDED, df = 4) +
    ns(RECORD_YR, df = 4) +
    ns(GIVING_MAX_CASH_YR, df = 4) +
    GIVING_MAX_CASH_MO +
    ns(GIVING_MAX_PLEDGE_YR, df = 4) +
    KSM_PROSPECT +
    HOUSEHOLD_REGION +
    ns(VISITS_5FY, df = 4) +
    ns(LOYAL_5_PCT_CASH, df = 2) +
    ns(LOYAL_5_PCT_ANY, df = 2) +
    UPGRADE3_CASH +
    ns(VELOCITY3_LIN_NGC, df = 4) +
    ns(GAVE_10K, df = 4) +
    SPOUSE_ALUM
...
lm_mses <- cbind(
  ...
  , lm_ridge_spline = c(
    NA
    , calc_mse(y = traindat$rv.amt, yhat = predict(lm_ridge_spline, s = 'lambda.1se', newdata = traindat))
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_ridge_spline, s = 'lambda.1se', newdata = holdoutdat))
  )
)
```
```{r}
...
LASSO can be interpreted as automatically performing variable selection, so only one model needs to be fitted.

```{r}
lm_lasso_spline <- cv.glmnet(
  rv.amt ~
    HOUSEHOLD_RECORD +
    PROGRAM_GROUP +
    PREF_ADDR_TYPE_CODE +
    ...
    SPOUSE_ALUM
  , data = traindat
  , alpha = 1 # 1 is the lasso penalizer
)
```
```{r}
plot(lm_lasso_spline)
```
...
lm_mses <- cbind(
  ...
  , lm_lasso_spline = c(
    NA
    , calc_mse(y = traindat$rv.amt, yhat = predict(lm_lasso_spline, s = 'lambda.1se', newdata = traindat))
    , calc_mse(y = holdoutdat$rv.amt, yhat = predict(lm_lasso_spline, s = 'lambda.1se', newdata = holdoutdat))
  )
)
```
...
Consider the out-of-sample performance of each method.

```{r}
lm_mses %>% mutate_if(is.numeric, round, 3) %>% print()
```

The lasso is outperformed by ridge regression, but the linear model with splines has comparable performance and is more interpretable.

# Conclusions

Looking at unoptimized models, basic logistic regression and linear regression with splines actually perform similarly to more complex penalized models, so I'd prefer to use them.

I initially proposed the following model:

$$ E \left( \text{giving, donor | covariates} \right) = E \left(\text{giving | donor, covariates} \right) P \left(\text{donor | covariates} \right) $$

If actual future giving is $Y$ and the prediction is $\hat{Y}$ then define overall prediction error as:

$$ e = Y_t - \hat{Y_t} = Y_t - E(\text{giving} | \text{gave}, X_t)P(\text{gave}|X_t) $$

```{r}
glm_dat <- modeling.data %>% mutate(
  YEARS_SINCE_FIRST_GIFT = 2016 - ifelse(GIVING_FIRST_YEAR > 0, GIVING_FIRST_YEAR, 2017)
  , YEARS_SINCE_ATHLETICS_TICKETS = 2016 - ifelse(ATHLETICS_TICKET_LAST > 0, ATHLETICS_TICKET_LAST, 2017)
  , YEARS_SINCE_MAX_CASH_YR = 2016 - ifelse(GIVING_MAX_CASH_YR > 0, GIVING_MAX_CASH_YR, 2017)
)
lm_dat <- modeling.data %>% mutate(
  # Create spouse flag
  SPOUSE_ALUM = ifelse(SPOUSE_FIRST_KSM_YEAR > 0, 'TRUE', 'FALSE') %>% factor()
  # Lump together little-used continents
...
error_compare <- data.frame(
  giving = modeling.data$rv.amt
  , log.giving = log10plus1(modeling.data$rv.amt)
  , gave = modeling.data$rv.gave
  , est.giving = predict(lm_splines, newdata = lm_dat, type = 'response')
  , est.gave = predict(glm_st_splines, newdata = glmdat, type = 'response')
) %>% mutate(
...
Note that training the predicted giving amounts model with donors only leads to a hugely inflated MSE when predicting giving amounts of people who turned out to be nondonors. Multiplying by the estimated probability of making a gift brings the MSE down to something reasonable as hoped.

This is a little hard to read. Here's what the predicted versus actual amounts look like when binned.
...
A few takeaways for the giving amount model:
...
Here are the expected values when binned.
...
Takeaways for the expected value ranking model:
...
Finally, let's look at expected value versus predicted giving.
...
We could come up with some rules of thumb, for instance anyone with predicted giving over `r scales::dollar(10^{tmpcutoff + 1})` is worth looking at, but otherwise filter out those with an EV score below `r tmpc2`.