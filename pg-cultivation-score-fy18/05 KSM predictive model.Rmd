  ...
  errs_out <- foreach(
    ...
    # Fit temp model, where alpha = 0 is the ridge regression penalty
      tmpmodel <- cv.glmnet(
        rv.gave ~
        .
        + log10plus1(GIVING_FIRST_YEAR_CASH_AMT) - GIVING_FIRST_YEAR_CASH_AMT
        + log10plus1(GIVING_FIRST_YEAR_PLEDGE_AMT) - GIVING_FIRST_YEAR_PLEDGE_AMT
        + log10plus1(GIVING_MAX_CASH_AMT) - GIVING_MAX_CASH_AMT
        + log10plus1(GIVING_MAX_PLEDGE_AMT) - GIVING_MAX_PLEDGE_AMT
        + log10plus1(GIVING_CASH_TOTAL) - GIVING_CASH_TOTAL
        + log10plus1(GIVING_PLEDGE_TOTAL) - GIVING_PLEDGE_TOTAL
        + log10plus1(GIVING_AF_TOTAL) - GIVING_AF_TOTAL
        + log10plus1(GIVING_CRU_TOTAL) - GIVING_CRU_TOTAL
        + sqrt(GIFTS_ALLOCS_SUPPORTED) - GIFTS_ALLOCS_SUPPORTED
        + sqrt(GIFTS_FYS_SUPPORTED) - GIFTS_FYS_SUPPORTED
        + sqrt(GIFTS_CASH) - GIFTS_CASH
        + sqrt(GIFTS_OUTRIGHTS_PAYMENTS) - GIFTS_OUTRIGHTS_PAYMENTS
        + log10plus1(CASH_PFY1) - CASH_PFY1
        + log10plus1(CASH_PFY2) - CASH_PFY2
        + log10plus1(CASH_PFY3) - CASH_PFY3
        + log10plus1(CASH_PFY4) - CASH_PFY4
        + log10plus1(CASH_PFY5) - CASH_PFY5
        + log10plus1(CRU_PFY1) - CRU_PFY1
        + log10plus1(CRU_PFY2) - CRU_PFY2
        + log10plus1(CRU_PFY3) - CRU_PFY3
        + log10plus1(CRU_PFY4) - CRU_PFY4
        + log10plus1(CRU_PFY5) - CRU_PFY5
        + sqrt(GIFT_CLUB_KLC_YRS) - GIFT_CLUB_KLC_YRS
        + sqrt(GIFT_CLUB_LOYAL_YRS) - GIFT_CLUB_LOYAL_YRS
        + sqrt(GIFT_CLUBS_CFY) - GIFT_CLUBS_CFY
        + sqrt(GIFT_CLUBS_PFY1) - GIFT_CLUBS_PFY1
        + sqrt(GIFT_CLUBS_PFY2) - GIFT_CLUBS_PFY2
        + log10plus1(EVALUATION_LOWER_BOUND) - EVALUATION_LOWER_BOUND
        + log10plus1(UOR_LOWER_BOUND) - UOR_LOWER_BOUND
        + sqrt(MONTHS_ASSIGNED) - MONTHS_ASSIGNED
        + sqrt(COMMITTEE_NU_DISTINCT) - COMMITTEE_NU_DISTINCT
