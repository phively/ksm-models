...
In this case, the intercept leads to a huge overall MSE. However, in the final model this will be reduced because most entities have a very low $P \left(\text{g level donor | covariates} \right)$
...
duc_data <- rbind(
  ...
  , data.frame(model = 'regression', duc_data_gen(predictions$reg.mod, predictions$truth))
  , data.frame(model = 'ev', duc_data_gen(predictions$ev, predictions$truth))
...
The probability model greatly outperforms the regression model. Furthermore, compare this plot to [previous results](https://phively.github.io/ksm-models/pg-cultivation-score-fy18/08%20Dollars%20Under%20the%20Curve.nb.html#dollars_under_the_curve). Fitting a model to just high-end donors results in greatly degraded performance compared to looking at all donors and gifts at any dollar level.