...
It's not immediately obvious how to go about comparing different techniques given that the usual accuracy metrics like classification error, RMSE, probability calibration, etc. are not directly comparable, not to mention hard classifiers and . However, since the difference in output between these different modeling techniques, heuristics, and algorithms ultimately comes down to sort order, why not compare that sort order directly?
