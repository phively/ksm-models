...
for (rep in 1:reps) {
  ...
  # Create cross-validation indices
  xv <- KFoldXVal(traindat, k = folds)
  prlout <- foreach (fold = 1:length(xv)
                     , .combine = rbind
                     , .packages = c('dplyr', 'splines')) %dopar% {
    # Fit temp model
    tmpmodel <- glm(GAVE_10K ~
                        ns(RECORD_YR, df = 5)
                      + PROGRAM_GROUP + PREF_ADDR_TYPE_CODE + HOUSEHOLD_CONTINENT
...
for (rep in 1:reps) {
  ...
  prlout <- foreach (fold = 1:length(xv)
    ...
    tmpmodel <- cv.glmnet(as.factor(GAVE_10K) ~
                           ns(RECORD_YR, df = 5)
                         + PROGRAM_GROUP + PREF_ADDR_TYPE_CODE + HOUSEHOLD_CONTINENT
                         + GIVING_PLEDGE_ANY
                         + GIVING_PLEDGE_FIRST_YR + GIFTS_ALLOCS_SUPPORTED
...
The penalized model apparently sacrifices precision for sensitivity.
...
mnpen <- glm(GAVE_10K ~
                      ...
                      , data = traindat
                      , family = binomial)
mpen <- cv.glmnet(as.factor(GAVE_10K) ~
                           ns(RECORD_YR, df = 5)
                         + PROGRAM_GROUP + PREF_ADDR_TYPE_CODE + HOUSEHOLD_CONTINENT
                         + GIVING_PLEDGE_ANY
                         + GIVING_PLEDGE_FIRST_YR + GIFTS_ALLOCS_SUPPORTED
...
```{r pen.coef.compare, warning = FALSE, message = FALSE, fig.height = 6}
neg_sqrt_trans <- function() scales::trans_new(
    'neg_sqrt' # Transformation nickname
  , function(x) {sign(x) * sqrt(abs(x))} # Negative square root
  , function(x) {sign(x) * x^2} # Inverse
)


full_join(
    data.frame(var = coef(mnpen) %>% names(), unpenalized = coef(mnpen))
  , data.frame(var = coef(mpen)[, 1] %>% names(), shrinkage = coef(mpen)[, 1])
...
The penalization appears to be fairly strong. Note that the penalized model included dummy coding for the factor base levels (i.e. not folded into the intercept term).

Also note in the nonpenalized model the velocity coefficients are on the order of 10E1 which is extremely high compared to the average.
...
theta <- sum(traindat$GAVE_10K) / nrow(traindat)

confus_test <- list(
    bad = conf_matrix(mbad, newdata = holdoutdat, threshold = theta)
  , baseline = conf_matrix(mbad, newdata = holdoutdat, threshold = theta)
  , trimmed = conf_matrix(mtrim3, newdata = holdoutdat, threshold = theta)
  , nonpenalized = conf_matrix(mnpen, newdata = holdoutdat, threshold = theta)
  , penalized = conf_matrix_glmnet(mpen, newdata = holdoutdat, threshold = theta)
)
```

## Results

```{r holdout.results.confmatrices}
with(confus_test,
  list(
      bad = bad$conf_matrix
    , baseline = baseline$conf_matrix
    , trimmed = trimmed$conf_matrix
    , nonpenalized = nonpenalized$conf_matrix
    , penalized = penalized$conf_matrix
  )
)
```

```{r holdout.results.table}
with(confus_test,
  rbind(
      bad = data.frame(error = bad$error, precision = bad$precision, sensitivity = bad$sensitivity, F1 = bad$F1_score)
    , baseline = c(baseline$error, baseline$precision, baseline$sensitivity, baseline$F1_score)
    , trimmed = c(trimmed$error, trimmed$precision, trimmed$sensitivity, trimmed$F1_score)
    , nonpenalized = c(nonpenalized$error, nonpenalized$precision, nonpenalized$sensitivity, nonpenalized$F1_score)
    , penalized = c(penalized$error, penalized$precision, penalized$sensitivity, penalized$F1_score)
  )
) %>% round(4)
```

Reassuringly, these results mirror those obtained during cross-validation.

The central question: prioritize low error or high sensitivity? This determines whether the nonpenalized or penalized model is "best." Also recall that the nonpenalized model leans very heavily on `VELOCITY3_LIN` which is a point against.

## Calibration

The basic idea: regress classification on fitted predictions and see if there are poorly-calibrated areas, e.g. of the points with a predicted probability around 30% only 10% are donors.

### Internal calibration

A.k.a. in-sample results

```{r int.cal.nonpen}
# Nonpenalized data frame
data.frame(
    class = traindat$GAVE_10K
  , prediction = predict(mnpen, newdata = traindat, type = 'response')
) %>%
# Plot results
ggplot(aes(x = prediction, y = class)) +
  ...
  labs(title = 'GAM smoother, nonpenalized coefficients', color = 'class')
```
...
data.frame(
    class = traindat$GAVE_10K
  , prediction = predict(mpen, newdata = traindat, type = 'response')
...
A.k.a. out-of-sample results

```{r ext.cal.nonpen}
# Nonpenalized data frame
data.frame(
    class = holdoutdat$GAVE_10K
  , prediction = predict(mnpen, newdata = holdoutdat, type = 'response')
) %>%
# Plot results
ggplot(aes(x = prediction, y = class)) +
  geom_point(aes(color = as.factor(class))) +
  ...
  geom_abline(slope = 1, intercept = 0) +
  labs(title = 'OOS Loess smoother, nonpenalized coefficients', color = 'class')
```
...
data.frame(
    class = holdoutdat$GAVE_10K
  , prediction = predict(mpen, newdata = holdoutdat, type = 'response')
) %>% mutate(prediction = X1) %>%
# Plot results
ggplot(aes(x = prediction, y = class)) +
  geom_point(aes(color = as.factor(class))) +
  geom_smooth(method = 'loess') +
  geom_abline(slope = 1, intercept = 0) +
  labs(title = 'OOS Loess smoother, penalized coefficients', color = 'class')
```

We have ourselves a finalist: the penalized model wins hands down.