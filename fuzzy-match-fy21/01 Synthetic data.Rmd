...
# Scramble data
set.seed(8926656)
tmp <- data_10k_records_scrambled
for(i in 1:20) {
  # Extract current row from scramble_mask
  sm <- scramble_mask[i, ]
  # Skip if no operations to perform
  ...
  for(j in 1:data_size[2]) {
    ...
    c('==Col: ', j) %>% print_debug()
    data_10k_with_typos[i, j] <- data_10k_records_scrambled[i, j] %>%
      string_scrambler(p = 0.02, debug = TRUE)
  }
}
```
...
data_10k %>%
  ...
  write_csv(
    path = 'generated data/Randomized 10k biodata - original.csv'
  )
...
data_10k_records_scrambled %>%
  ...
  ReplaceValues(new.val = '') %>%
  write_csv(
    path = 'generated data/Randomized 10k biodata - scrambled fields.csv'
  )
...
data_10k_with_typos %>%
  ...
  ReplaceValues(new.val = '') %>%
  write_csv(
    path = 'generated data/Randomized 10k biodata - scrambled fields and typos.csv'
  )
```