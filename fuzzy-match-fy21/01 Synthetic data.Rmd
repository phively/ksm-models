...
string_scrambler <- function(
  ...
  # Transposition (with next character, if present)
  f_transposition <- function(input_char, next_char) {
    next_char %>%
    paste0(input_char) %>%
      return()
  }
  ...
  scrambler <- for(i in 1:string_len) {
    ...
    next_char <- case_when(
      ...
      , ops[i] == 'transposition' ~
          f_transposition(input_char, next_char = str_sub(string, i + 1, i + 1))
      # Fallback
      , TRUE ~ input_char
    )
...
field_scrambler <- function(
  ...
  , operation # name of permissible operations
