...
```{python}
# Add a cutoff time
fm_giv, features = ft.dfs(
  entityset = py_giv
  , target_entity = 'households'
  , agg_primitives = ['count', 'sum', 'std', 'mean', 'max', 'min', 'median', 'first', 'last', 'percent_true']
  , trans_primitives = ['cum_sum', 'cum_max', 'month', 'year', 'subtract_numeric', 'divide_numeric', 'time_since_previous']
  , cutoff_time = pd.Timestamp('2019-01-01 00:00:00')
...
```{r}
dfs_giv_cutoff <- py$py_giv %>% dfs(
   target_entity = 'households'
   , agg_primitives = c('count', 'sum', 'std', 'mean', 'max', 'min', 'median', 'first', 'last', 'percent_true')
   , trans_primitives = c('cum_sum', 'cum_max', 'month', 'year', 'subtract_numeric', 'divide_numeric', 'time_since_previous')
   , cutoff_time = py$pd$Timestamp('2019-01-01 00:00:00') # Can't get this to work
   , cutoff_time_in_index = TRUE
   , max_depth = 2
   , verbose = TRUE
)
...
# py$fm_giv %>% filter(`COUNT(giv_transactions)` > 0) %>% View()
```


## Interesting values test
